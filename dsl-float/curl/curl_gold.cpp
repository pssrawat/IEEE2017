...
(float4* l_input, float dx, float dy, float dz, int height, int width_y,
 int width_x, float4* l_output)
{
  float4 (*curl1)[width_y][width_x] =
    (float4 (*)[width_y][width_x])new float4[height*width_y*width_x];
  memset(curl1, 0, sizeof(float4)*height*width_y*width_x);
  float4 (*input)[width_y][width_x] = (float4 (*)[width_y][width_x])l_input;
  float4 (*output)[width_y][width_x] = (float4 (*)[width_y][width_x])l_output;
