...
__global__ void heat (double * __restrict__ in, double * __restrict__ out1, int L, int M, int N) {
  ...
  //Declarations
  double reg_in_m1=0, __shared__ sh_in_c0[16][32], reg_in_p1=0;
  double reg_out_m2=0, __shared__ sh_out_m1[16][32], reg_out_c0=0;
...
extern "C" void host_code (double *h_in, double *h_out2, int L, int M, int N) {
  double *in;
  ...
  cudaMemcpy (h_out2, out2, sizeof(double)*L*M*N, cudaMemcpyDeviceToHost);
}
