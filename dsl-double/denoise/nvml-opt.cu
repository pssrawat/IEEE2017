...
extern "C" void host_code (double *h_u, double *h_f, double gamma, double r, double sigma2, int L, int M, int N) {
	double *u;
	cudaMalloc (&u, sizeof(double)*L*M*N);
	check_error ("Failed to allocate device memory for u\n");
	cudaMemcpy (u, h_u, sizeof(double)*L*M*N, cudaMemcpyHostToDevice);
	double *f;
	cudaMalloc (&f, sizeof(double)*L*M*N);
	check_error ("Failed to allocate device memory for f\n");
	cudaMemcpy (f, h_f, sizeof(double)*L*M*N, cudaMemcpyHostToDevice);
	double *out;
	...
	cudaMemcpy (h_u, u, sizeof(double)*L*M*N, cudaMemcpyDeviceToHost);
}
